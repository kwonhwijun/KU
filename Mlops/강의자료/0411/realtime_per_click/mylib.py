...
def db_to_df_random(db_name, table_name): ## db에서 하나의 행을 랜덤하게 받아와서 df형태로 출력
    con = sqlite3.connect(db_name)
    cur = con.cursor()
    query = cur.execute("SELECT count(*) FROM " + table_name)
    cnt = query.fetchall()[0][0] ## 현재 테이블의 행의 수
    rid = str(np.random.randint(0,cnt,1)[0]) ## 현재 테이블의 랜덤한 행번호 하나
